...
public:
    ...
    Sphere(std::string name, vec3 center= vec3(0., 0., 0.), double radius=1., char *path=(char*) "") : Object(name), center(center), radius(radius), path(path) {
        if(strcmp(path,"")==0){
...
private:
    ...
    bool delimitation(vec4 p0, vec4 V, vec4 p);
    vec4 point;
    vec3 normal;
