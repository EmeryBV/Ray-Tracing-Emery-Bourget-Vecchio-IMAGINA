...
//Recursion depth for raytracer_template
int maxDepth = 7;
...
double dist(vec4 v1, vec4 v2) {
    vec4 vec = v2 - v1;
    return sqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z);
}

/*Mettre à jour castRay() pour ajouter des ombres :
 * si l’objet intersecté est dans l’ombre il
ne doit pas être illuminé (black). (15 pts)*/
/* -------------------------------------------------------------------------- */
bool shadowFeeler(vec4 p0, Object *object) {
    float dist_lum = dist(p0, lightPosition);
    float dist_min = std::numeric_limits<double>::infinity();
    vec4 E = normalize(lightPosition - p0);
    for (int i = 0; i < sceneObjects.size(); i++) {
        if (sceneObjects[i] != object) {
            Object::IntersectionValues objectValue = sceneObjects[i]->intersect(p0, E);
            if (objectValue.t < dist_min && objectValue.t > EPSILON ) {
                dist_min = objectValue.t;
            }
        }
    }

    return (dist_min < dist_lum);
}

double shadowFeelerSmooth(vec4 p0, Object *object) {
    float dist_lum  = 0.0;
    float dist_min = std::numeric_limits<double>::infinity();
    float r = 0.01;

    double nb_cycle = 20;
    double shadow = 60.0*nb_cycle;
    double discretisation = 360.0/60.0 ;
    Object::IntersectionValues nearobjectValue;
    double nbr_ite = shadow;
        for (int i = 0; i <nb_cycle ; ++i) {
            r+=0.05;
            for (int i = 0; i < 360; i+= discretisation) {
                vec4 areaPosition = vec4(lightPosition.x + r* cos(i), lightPosition.y,
                                         lightPosition.z + r * sin(i),
                                         lightPosition.w);
                dist_lum = length(p0- areaPosition);
                vec4 E = normalize(areaPosition - p0 );
                for (int z = 0; z < sceneObjects.size(); z++) {
                        Object::IntersectionValues objectValue = sceneObjects[z]->intersect(p0, E);
                        if (objectValue.t < dist_min && (sceneObjects[z] != object) ||  objectValue.t > EPSILON && (sceneObjects[z] == object)) {
                            nearobjectValue = objectValue;
                            dist_min = objectValue.t;
                        }
                }

//                std::cout << "dist_min:  " << dist_min << "\n";
//                std::cout << "dist_lum:"  <<dist_lum << "\n\n";
                if (dist_min < dist_lum)shadow = shadow - 0.7;
//                else std::cout << "Je suis la "<< "\n";
            }
        }

        return shadow  /nbr_ite ;
}
...
vec4 castRay(vec4 p0, vec4 E, Object *lastHitObject, int depth) {
    ...
    Object::IntersectionValues nearObjectValues;
    double min = std::numeric_limits<double>::infinity();
    Object::IntersectionValues objectValue;

    Object *nearObject;
    double t = min;
    for (Object *o  : sceneObjects) {
        objectValue = o->intersect(p0, E);
        if (objectValue.ID_ != 0 ) {
            t = objectValue.t;

            if (t < min) {
                min = t;
                nearObject = o;
                nearObjectValues = objectValue;
...
//////////////////////////////////////////////////OMBRES DURES//////////////////////////////////////////////////
//    if (shadowFeeler(nearObjectValues.P, nearObject))
...
//////////////////////////////////////////////////OMBRES DOUCES//////////////////////////////////////////////////
    ...
    //ombres douces
    if (shadowFeeler(nearObjectValues.P, nearObject))
    coef = shadowFeelerSmooth(nearObjectValues.P, nearObject);



    vec4 direction = normalize(vec3(E.x, E.y, E.z));

    vec4 P = nearObjectValues.P; P.w = 1;
    vec4 L = lightPosition - P; //TO LIGHT
    L = normalize(L);
    vec4 C = cameraPosition - P; //TO CAMERA
    C = normalize(-C);
    vec4 N = nearObjectValues.N;
    N = normalize(N);
    vec4 V = normalize(E); V.w = 1;
    N.w = 0; L.w = 0; C.w = 0;

//    vec3 R = normalize(reflect(-C, normal));

    vec4 rayReflectionRec = normalize(reflect(E, nearObjectValues.N));
//

//////////////////////////////////////////////////PHONG//////////////////////////////////////////////////
    color4 material_ambient(nearObject->shadingValues.color.x * nearObject->shadingValues.Ka,
                            nearObject->shadingValues.color.y * nearObject->shadingValues.Ka,
                            nearObject->shadingValues.color.z * nearObject->shadingValues.Ka, 1.0);

    color4 material_diffuse(nearObject->shadingValues.color.x * nearObject->shadingValues.Kd,
                            nearObject->shadingValues.color.y * nearObject->shadingValues.Kd,
                            nearObject->shadingValues.color.z * nearObject->shadingValues.Kd, 1.0);

    color4 material_specular(nearObject->shadingValues.Ks,
                             nearObject->shadingValues.Ks,
                             nearObject->shadingValues.Ks, 1.0);

    float material_shininess = nearObject->shadingValues.Kn;


    vec4 R = normalize(reflect(L, N));

    color4 ambient_product = GLState::light_ambient * material_ambient; ambient_product.w = 1.0;
    color4 diffuse_product = GLState::light_diffuse * material_diffuse *  dot(N, L);
    diffuse_product.w = 1.0;

    color4 specular_product = GLState::light_specular * material_specular *
                              pow( std::max(0.0f,dot(R, V)), material_shininess);
    specular_product.w = 1.0;

    color = (ambient_product + diffuse_product + specular_product ) * coef;
    color.w = 1.0;
//////////////////////////////////////////////////MIROIR//////////////////////////////////////////////////
    if (nearObject->shadingValues.Ks > 0 and depth != maxDepth ) {

        vec4 miroir = normalize(reflect(E,N));//Réflection
        color = castRay(nearObjectValues.P, miroir, nearObject, depth + 1) * ( nearObject->shadingValues.Ks);

    }

//////////////////////////////////////////////////TRANSPARANCE//////////////////////////////////////////////////
    if (nearObject->shadingValues.Kt > 0) {
        double n1, n2;
        if (lastHitObject == nearObject) {
            n1 = nearObject->shadingValues.Kr;
            n2 = 1;
        }
        else {
            n1 = 1;
            n2 = nearObject->shadingValues.Kr;

        }
        float  nu = n1/n2;

        float etai = 1, etat =  nearObject->shadingValues.Kr;
       vec4 n = N  ;
        ...
        float nR =etai /etat  ;
        float c2 = sqrt(1 - pow(nR,2)* pow((1-cos(dot(V,n))),2));

        vec4 refract = normalize(nu * E + (nu*c1 - c2)*n);
        color =castRay(nearObjectValues.P, refract, nearObject, depth + 1);
    }
    color.w = 1.0;
...
void initCornellBox() {
    ...
    { //Left Wall
        ...
        Object::ShadingValues _shadingValues;
        _shadingValues.color = vec4(1.0, 0.0, 100, 1.0);
        _shadingValues.Ka = 0.0;
        _shadingValues.Kd = 1.0;
        _shadingValues.Ks = 0.0;
        _shadingValues.Kn = 16.0;
    ...
    { //Floor
        sceneObjects.push_back(new Square("Floor", RotateX(-90) * Translate(0.0, 0.0, -2.0) * Scale(3.0, 2.0, 1.0)));
        Object::ShadingValues _shadingValues;
        _shadingValues.color = vec4(1.0, 1.0, 1.0, 1.0);
        _shadingValues.Ka = 0.0;
        _shadingValues.Kd = 1.0;
    ...
    { //Ceiling
        sceneObjects.push_back(new Square("Ceiling", RotateX(90) * Translate(0.0, 0.0, -2.0) * Scale(3.0, 2.0, 1.0)));
        Object::ShadingValues _shadingValues;
        _shadingValues.color = vec4(1.0, 1.0, 1.0, 1.0);
        _shadingValues.Ka = 0.0;
        _shadingValues.Kd = 1.0;
    ...
    {
//        sceneObjects.push_back(new Sphere("Glass sphere", vec3(1.0, -1.25, 0.5),0.75,"./mesh/glass_sphere.obj"));
        sceneObjects.push_back(new Sphere("Glass sphere", vec3(1.0, -1.25, 0.5),0.75));
        Object::ShadingValues _shadingValues;
        _shadingValues.color = vec4(1.0,1.0,1.0,1.0);
        _shadingValues.Ka = 0.7;
        _shadingValues.Kd = 0.2;
        _shadingValues.Ks = 0.7;
        _shadingValues.Kn = 16.0;
    ...
    {
//        sceneObjects.push_back(new Sphere("Mirrored Sphere", vec3(-1.0, -0.5, -1.25), 0.75,"./mesh/mirrored_sphere.obj"));
        sceneObjects.push_back(new Sphere("Mirrored Sphere", vec3(-1.0, -.5 , -1.25), 0.75));
        Object::ShadingValues _shadingValues;
        _shadingValues.color = vec4(1.0, 1.0, 1.0, 1.0);
        _shadingValues.Ka = 0.7;
        _shadingValues.Kd = 0.2;
...
void initUnitSphere() {
...

    {
        ...
        sceneObjects.push_back(new Sphere("Diffuse sphere", vec3(1.0, 0.0, 0.0), 1));
        Object::ShadingValues _shadingValues;

        _shadingValues.color = vec4(2.0, 0.0, 0.0, 1.0);
    ...
    sceneObjects.push_back(new Sphere("Diffuse sphere green", vec3(-1.0, 0.0, 0.0), 1.0));
    Object::ShadingValues _shadingValues;
...
void drawObject(Object *object, GLuint vao, GLuint buffer) {
    ...
    color4 material_diffuse(object->shadingValues.color.x * object->shadingValues.Kd,
                            object->shadingValues.color.y * object->shadingValues.Kd,
                            object->shadingValues.color.z * object->shadingValues.Kd, 1.0);
